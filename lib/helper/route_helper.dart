...
class RouteHelper {
  ...
  static String getChatRoute({@required OrderModel orderModel, @required bool isStore}) {
    String _orderModel = base64Encode(utf8.encode(jsonEncode(orderModel)));
    return '$chatScreen?order=$_orderModel&page=${isStore ? 'restaurant' : 'admin/deliveryMan'}';
  }
  ...
  static List<GetPage> routes = [
    ...
    GetPage(name: chatScreen, page: () {
      final _orderModel = jsonDecode(utf8.decode(base64Url.decode(Get.parameters['order'].replaceAll(' ', '+'))));
      return getRoute(ChatScreen(orderModel : _orderModel != null ? OrderModel.fromJson(_orderModel) : null, isStore: Get.parameters['page'] == 'restaurant'));
      }),
  ];
