...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void initState() {
    super.initState();
    selectedDate;
    _dateController.text = DateFormat.yMd('es').format(DateTime.now());
    _timeController.text = DateFormat('hh:mm a').format(DateTime.now());
    _dateTimeController.text = _dateController.text +' - '+ _timeController.text;
    dateTimeFormat = '${DateFormat('yyyy-MM-dd').format(selectedDate)}';
    DateTime now = DateTime.tryParse(dateTimeFormat);
    convertedDateTime = "${now.year.toString()}/${now.month.toString().padLeft(2,'0')}/${now.day.toString().padLeft(2,'0')} ${selectedDate.hour.toString().padLeft(2,'0')}:${selectedDate.minute.toString().padLeft(2,'0')}";
    ...
    if (_isLoggedIn) {
      ...
      _codDisable = Get.find<StoreController>().storeModel.stores[0].codDisabled;
    }
  }
  ...
  Future<Null> _selectDate(BuildContext context) async {
    ...
    if (picked != null) _selectTime(context);
    setState(() {
      selectedDate = picked;
      if(selectedDate != null) {
        _dateController.text = DateFormat.yMd('es').format(selectedDate);
      }
    });
  }
  ...
  Future<Null> _selectTime(BuildContext context) async {
    ...
    if (picked != null)
      setState(() {
        ...
        dateTimeFormat = '${DateFormat('yyyy-MM-dd').format(selectedDate)}';
        DateTime now = DateTime.tryParse(dateTimeFormat);
        convertedDateTime = "${now.year.toString()}/${now.month.toString().padLeft(2,'0')}/${now.day.toString().padLeft(2,'0')} ${_hour.toString().padLeft(2,'0')}:${_minute.toString().padLeft(2,'0')}";
            ...
            if (!_isCashOnDeliveryActive &&
              ...
              orderController.placeOrder(
                  PlaceOrderBody(
                    ...
                    chargePayer: null,
                    dmTips: _tipController.text.trim(), datetimeval: convertedDateTime,
                  ),
                  _callback);
            }
          })
